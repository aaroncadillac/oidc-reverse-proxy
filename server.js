...
prog
  .version(pkg.version)
  .option('--host, -H,', '$OIDC_RPROXY_HOST Host to listen on', process.env.OIDC_RPROXY_HOST || '127.0.0.1')
  .option('--port, -p', '$OIDC_RPROXY_PORT Port to listen on', process.env.OIDC_RPROXY_PORT || 3000)
  .option('--config, -c', 'Provide path to custom config', '~/oidc-rproxy.json');
